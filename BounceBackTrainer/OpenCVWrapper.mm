...
static AdaptiveRanges getAdaptiveRanges(float avgBrightness) {
  ...
  if (avgBrightness > 150) { // Bright outdoor lighting
    ranges.yellowLow =
        cv::Scalar(10, 50, 80); // Much more aggressive for outdoor
    ...
    ranges.redLow2 =
        cv::Scalar(150, 70, 70); // Much more aggressive for outdoor
...
struct Target {
  ...
  Target()
      : boundingBox(-1, -1, -1, -1), isCircular(false), targetNumber(0),
...
struct BallTracker {
  ...
  BallTracker()
      : position(-1, -1), velocity(0, 0), velocityMagnitude(0),
        isDetected(false), framesDetected(0), framesWithoutDetection(0),
        confidence(0.0) {}
};
...
static const int MAX_BALL_AREA =
    50000; // Increased maximum area for larger balls
static const double MIN_CIRCULARITY =
    0.5; // Reduced circularity requirement (was too strict)
static const double MIN_CONTRAST =
    15.0; // Reduced contrast requirement for low-light conditions
static const int MIN_CONSECUTIVE_DETECTIONS =
    3; // Require multiple consecutive detections
...
static cv::Point
detectBallByMotion(const cv::Mat &gray,
                   cv::Ptr<cv::BackgroundSubtractorMOG2> pMOG2);
static bool detectSoccerBall(const cv::Mat &frame, cv::Point &ballCenter,
                             float &ballRadius);
static std::vector<Target> detectTargets(const cv::Mat &frame,
                                         std::vector<cv::Rect> &tapeRects);
static std::vector<Target>
detectCircularTargets(const cv::Mat &frame,
                      const ColorThresholds::AdaptiveRanges &adaptiveRanges);
static std::vector<Target>
detectTargetsByContours(const cv::Mat &frame,
                        const ColorThresholds::AdaptiveRanges &adaptiveRanges);
static bool isColorMatch(const cv::Mat &hsv, const cv::Point &center,
                         int radius, const cv::Scalar &low,
                         const cv::Scalar &high, double minPercentage);
static void drawDetectedTarget(cv::Mat &frame, const Target &target);
static cv::Rect getSafeROI(const cv::Mat &frame, const cv::Point &center,
                           int radius);
...
// Helper to run the same detection pipeline as analyzeVideo, but for a single
// frame
static void analyzeSingleFrame(const cv::Mat &frame, cv::Rect goalRegion,
                               std::vector<Target> &outTargets,
                               BallTracker &outBallTracker) {
  std::vector<cv::Rect> tapeRects;
  ...
  if (detectedPosition.x < 0) {
    ...
    cv::Point motionBall =
        detectBallByMotion(gray, cv::createBackgroundSubtractorMOG2());
    if (motionBall.x >= 0) {
  ...
  if (detectedPosition.x >= 0 && detectedPosition.y >= 0) {
    ...
    if (recentBallPositions.size() >= MIN_CONSECUTIVE_DETECTIONS) {
      ...
      for (size_t i = 1; i < recentBallPositions.size(); ++i) {
        double distance =
            cv::norm(recentBallPositions[i] - recentBallPositions[i - 1]);
      ...
      if (isConsistent) {
        ...
        NSLog(
            @"[OpenCV] Ball detection validated with %zu consecutive positions",
            recentBallPositions.size());
      } else {
      ...
      outBallTracker.framesWithoutDetection++;
      NSLog(@"[OpenCV] Ball detection pending - need %d consecutive "
            @"detections, have %zu",
  ...
  double velocity = 0.0;
  if (ballTracker.position.x >= 0 && ballTracker.position.y >= 0 &&
      detectedPosition.x >= 0 && detectedPosition.y >= 0) {
    velocity =
        cv::norm(cv::Point2d(detectedPosition.x - ballTracker.position.x,
                             detectedPosition.y - ballTracker.position.y));
  }
  if (outBallTracker.isDetected &&
      (velocity < 2.0 || (currentFrame - lastValidFrame < cooldownFrames))) {
    outBallTracker.isDetected = false;
  } else if (outBallTracker.isDetected) {
    lastValidFrame = currentFrame;
    ballTracker.position = detectedPosition;
    ballTracker.velocity =
        cv::Point2d(detectedPosition.x - ballTracker.position.x,
                    detectedPosition.y - ballTracker.position.y);
    ballTracker.velocityMagnitude = velocity;
  }
  NSLog(@"[OpenCV] Ball velocity: %f, Detected: %d", velocity,
        outBallTracker.isDetected);

  // Compute ball confidence
  double ballConfidence = 0.0;
  if (outBallTracker.isDetected) {
    ballConfidence = std::min(
        1.0, std::min(detectedPosition.x / static_cast<double>(frame.cols),
                      detectedPosition.y / static_cast<double>(frame.rows)));
  ...
  if (outBallTracker.confidence < minConfidenceThreshold) {
    NSLog(@"[OpenCV] Rejected ball due to low confidence: %.2f",
          outBallTracker.confidence);
    outBallTracker.isDetected = false;
  }
}
...
+ (NSDictionary *)detectTargetsInFrame:(UIImage *)frame
                            goalRegion:(CGRect)goalRegion {
  if (!frame)
    return @{};
  ...
  return @{
    @"targets" : targetArray,
    @"tapeRegion" : [NSValue valueWithCGRect:tapeRegion]
  };
}
...
+ (NSDictionary *_Nullable)detectBallInFrame:(UIImage *)frame {
  ...
  NSLog(@"[OpenCV] detectBallInFrame called with frame size: %dx%d",
        cvFrame.cols, cvFrame.rows);
  ...
  if (found) {
    NSLog(@"[OpenCV] Soccer ball found at (%d, %d) with radius %.1f",
  ...
  if (tracker.position.x >= 0 && tracker.position.y >= 0) {
    NSLog(@"[OpenCV] Fallback: Ball detected at (%d, %d)", tracker.position.x,
          tracker.position.y);
    NSLog(@"[Ball Detection] Velocity: %.2f, Confidence: %.2f, Frames: %d",
          tracker.velocityMagnitude, tracker.confidence,
          tracker.framesDetected);
  }
...
+ (NSDictionary *_Nullable)detectSoccerBall:(UIImage *)frame {
  ...
  NSLog(@"[OpenCV] detectSoccerBallInFrame called with frame size: %dx%d",
        cvFrame.cols, cvFrame.rows);
  ...
  if (found) {
    ...
    result[@"isDetected"] = @(YES);
    result[@"confidence"] = @(1.0);
    NSLog(@"[OpenCV] Soccer ball detected at (%d, %d) with radius %.1f",
          ballCenter.x, ballCenter.y, ballRadius);
  } else {
...
+ (BOOL)detectImpactWithBall:(NSDictionary *)ball
...
+ (NSArray<NSDictionary *> *)detectMotionInFrame:(UIImage *)frame {
  ...
  static cv::Ptr<cv::BackgroundSubtractorMOG2> pMOG2 =
...
+ (void)setProcessingMode:(NSString *)mode {
  if ([mode isEqualToString:@"fast"] || [mode isEqualToString:@"accurate"] ||
...
+ (void)calibrateForLighting:(UIImage *)frame {
  ...
  trackingStats[@"lastCalibrationTime"] =
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
          ...
          NSLog(@"[OpenCV] Video properties - Width: %d, Height: %d, FPS: %f",
                width, height, fps);
          ...
          cv::VideoWriter writer(output,
                                 cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                                 fps, cv::Size(width, height));
          if (!writer.isOpened()) {
            NSLog(@"Error: Could not create output video writer. Path: %s, "
                  @"Size: %dx%d, FPS: %f",
                  output.c_str(), width, height, fps);
            return;
          }
          NSLog(
              @"[OpenCV] Output writer created. Path: %s, Size: %dx%d, FPS: %f",
              output.c_str(), width, height, fps);
          ...
          while (cap.read(frame)) {
            ...
            if (!targetsDetected &&
                targetDetectionAttempts < MAX_DETECTION_ATTEMPTS) {
              std::vector<Target> detectedTargets =
                  detectTargets(frame, tapeRects);
              ...
              // If we have BOTH the boundary AND the targets, we can finalize
              // scene setup
              if (goalBoundaryLocked && !targets.empty()) {
                ...
                for (auto &target : targets) {
                  if (target.quadrant == 0) { // Assign only once
                    cv::Point center =
                        target.isCircular
                            ? target.center
                            : cv::Point(target.boundingBox.x +
                                            target.boundingBox.width / 2,
                                        target.boundingBox.y +
                                            target.boundingBox.height / 2);
                    target.quadrant = getQuadrant(center, goalBoundary);
                    NSLog(@"[OpenCV] Target %d assigned to quadrant %d",
                          target.targetNumber, target.quadrant);
                  }
                }
            ...
            if (goalBoundaryLocked) {
              cv::rectangle(outputFrame, goalBoundary, cv::Scalar(0, 255, 0),
                            2);
            }
            ...
            if (currentBallPosition.x >= 0 && currentBallPosition.y >= 0) {
              cv::circle(outputFrame, currentBallPosition, 8,
                         cv::Scalar(0, 0, 255), -1);
              cv::putText(outputFrame, "BALL (" + detectionMethod + ")",
                          cv::Point(currentBallPosition.x + 10,
                                    currentBallPosition.y - 10),
              ...
              if (bestTargetIndex >= 0) {
                ...
                cv::putText(outputFrame,
                            "HIT TARGET " +
                                std::to_string(bestTarget.targetNumber) + "!",
            ...
            cv::putText(outputFrame,
                        "Targets Detected: " + std::to_string(targets.size()),
                        cv::Point(10, height - 80), cv::FONT_HERSHEY_SIMPLEX,
                        0.5, cv::Scalar(255, 255, 255), 1);

            // Draw frame info
            cv::putText(outputFrame, "Frame: " + std::to_string(frameCount),
                        cv::Point(10, height - 60), cv::FONT_HERSHEY_SIMPLEX,
                        0.5, cv::Scalar(255, 255, 255), 1);
            cv::putText(outputFrame,
                        "Ball Detected: " +
                            std::string(ballDetected ? "YES" : "NO"),
                        cv::Point(10, height - 40), cv::FONT_HERSHEY_SIMPLEX,
                        0.5, cv::Scalar(255, 255, 255), 1);
            cv::putText(outputFrame, "Detection: " + detectionMethod,
                        cv::Point(10, height - 20), cv::FONT_HERSHEY_SIMPLEX,
                        0.5, cv::Scalar(255, 255, 255), 1);
            ...
            if (targetsDetected) {
              ...
              std::string lightingCondition =
                  (avgBrightness > 150)   ? "Bright Outdoor"
                  : (avgBrightness > 100) ? "Moderate"
                                          : "Indoor";
              cv::putText(outputFrame,
                          "Lighting: " + lightingCondition + " (" +
                              std::to_string((int)avgBrightness) + ")",
                          cv::Point(width - 300, height - 20),
            ...
            if (goalBoundaryLocked) {
              ...
              cv::line(outputFrame, cv::Point(midX, goalBoundary.y),
                       cv::Point(midX, goalBoundary.y + goalBoundary.height),
                       cv::Scalar(255, 255, 255), 1);
              cv::line(outputFrame, cv::Point(goalBoundary.x, midY),
                       cv::Point(goalBoundary.x + goalBoundary.width, midY),
                       cv::Scalar(255, 255, 255), 1);
            }
          ...
          cap.release();
          writer.release();
          NSLog(@"[OpenCV] Video processing completed. Total frames: %d",
                frameCount);
        }
      });
}
...
cv::BORDER_CONSTANT, cv::Scalar::all(0));
    ...
    cv::Mat planes[] = {cv::Mat_<float>(padded), cv::Mat::zeros(padded.size(),
CV_32F)}; cv::Mat complexI; cv::merge(planes, 2, complexI);
    ...
    int filterRadius = std::min(cx, cy) / 6; // Smaller filter for more
precision cv::circle(filter, cv::Point(cx, cy), filterRadius, cv::Scalar(1.0),
-1); cv::multiply(magI, filter, magI);
    ...
    int minRadius = std::max(5, (int)(std::min(cvFrame.rows, cvFrame.cols) *
0.02)); int maxRadius = std::min(150, (int)(std::min(cvFrame.rows, cvFrame.cols)
* 0.3));

    cv::HoughCircles(mag8U, circles, cv::HOUGH_GRADIENT, 1, mag8U.rows/8, 120,
25, minRadius, maxRadius);
    ...
    if (!circles.empty()) {
        ...
        for (const auto& circle : circles) {
            ...
            if (x >= 0 && x < cvFrame.cols && y >= 0 && y < cvFrame.rows &&
                ...
                cv::Rect roi = cv::Rect(std::max(0, x - radius), std::max(0, y -
radius), std::min(2 * radius, cvFrame.cols - std::max(0, x - radius)),
                                      std::min(2 * radius, cvFrame.rows -
std::max(0, y - radius)));
                ...
                if (roi.width > 0 && roi.height > 0) {
                    ...
                    cv::Mat roiBinary;
                    cv::threshold(roiMat, roiBinary, 0, 255, cv::THRESH_BINARY |
cv::THRESH_OTSU);

                    std::vector<std::vector<cv::Point>> contours;
                    cv::findContours(roiBinary, contours, cv::RETR_EXTERNAL,
cv::CHAIN_APPROX_SIMPLE);

                    double circularity = 0.0;
                    if (!contours.empty()) {
                        // Find the largest contour
                        auto largestContour = std::max_element(contours.begin(),
contours.end(),
                            [](const std::vector<cv::Point>& a, const
std::vector<cv::Point>& b) { return cv::contourArea(a) < cv::contourArea(b);
                            });

                        if (largestContour != contours.end()) {
                            double area = cv::contourArea(*largestContour);
                            double perimeter = cv::arcLength(*largestContour,
true); if (perimeter > 0) { circularity = 4 * M_PI * area / (perimeter *
perimeter);
                            }
                        }
                    }
                    ...
                    double contrastScore = std::min(1.0, stdDev / 50.0); //
Normalize contrast double sizeScore = 1.0 - std::abs(radius - (minRadius +
maxRadius) / 2.0) / ((maxRadius - minRadius) / 2.0); double circularityScore =
circularity;

                    // Combined confidence score
                    double confidence = (contrastScore * 0.4 + sizeScore * 0.3 +
circularityScore * 0.3);

                    // Debug logging
                    NSLog(@"[OpenCV][FFT] Circle candidate: pos(%d,%d)
radius=%d, stdDev=%.2f, circularity=%.3f, confidence=%.3f", x, y, radius,
stdDev, circularity, confidence);

                    // Final validation: stricter thresholds to reduce false
positives if (stdDev > MIN_CONTRAST && circularity > MIN_CIRCULARITY &&
confidence > 0.5) {
                        // Additional validation: check if this position is
consistent with recent detections bool isConsistent = true; if
(!recentBallPositions.empty()) { double avgDistance = 0.0; int validPositions =
0; for (const auto& pos : recentBallPositions) { double distance =
cv::norm(cv::Point2d(x, y) - cv::Point2d(pos.x, pos.y)); if (distance < 100) {
// Within 100 pixels avgDistance += distance; validPositions++;
                                }
                            }
                            if (validPositions > 0) {
                                avgDistance /= validPositions;
                                isConsistent = (avgDistance < 50); // Must be
close to recent positions
                            }
                        }
                        ...
                        if (isConsistent) {
                            ...
                            NSLog(@"[OpenCV][FFT] Rejected: position
inconsistent with recent detections");
                        }
                    } else {
                        NSLog(@"[OpenCV][FFT] Rejected: stdDev=%.2f (need>%.1f),
circularity=%.3f (need>%.3f), confidence=%.3f (need>0.5)", stdDev, MIN_CONTRAST,
circularity, MIN_CIRCULARITY, confidence);
                    }
                }
            }
        }
        ...
        if (validBallDetected) {
            ...
            NSLog(@"[OpenCV][FFT] Ball detected at (%d, %d) with radius %d,
confidence %.3f, history size: %zu", x, y, radius, bestConfidence,
recentBallPositions.size());
        }
    }
...
+ (NSDictionary *_Nullable)detectBallUnified:(UIImage *)frame {
  ...
  NSLog(@"[OpenCV] detectBallUnified called with frame size: %dx%d",
        cvFrame.cols, cvFrame.rows);
  ...
  @try {
    ...
    if (soccerBallFound && ballCenter.x >= 0 && ballCenter.y >= 0) {
      ...
      NSLog(@"[OpenCV] Soccer ball detection succeeded at (%d, %d) with radius "
            @"%.1f",
            ballCenter.x, ballCenter.y, ballRadius);
    ...
    NSLog(@"[OpenCV] ERROR: Soccer ball detection failed with exception: %@",
          exception.reason);
  }

  // Method 2: Try shape detection (HoughCircles) if soccer ball detection
  // failed
  if (bestConfidence < 0.7) {
    @try {
      ...
      if (shapeBall.x >= 0) {
        ...
        NSLog(@"[OpenCV] Shape detection succeeded at (%d, %d)", shapeBall.x,
              shapeBall.y);
      ...
      NSLog(@"[OpenCV] ERROR: Shape detection failed with exception: %@",
            exception.reason);
    }
  }
  ...
  if (bestConfidence < 0.5) {
    @try {
      ...
      if (colorBall.x >= 0) {
        ...
        NSLog(@"[OpenCV] Color detection succeeded at (%d, %d)", colorBall.x,
              colorBall.y);
      ...
      NSLog(@"[OpenCV] ERROR: Color detection failed with exception: %@",
            exception.reason);
    }
  }
  ...
  if (bestConfidence < 0.3) {
    @try {
      ...
      if (motionBall.x >= 0) {
        ...
        NSLog(@"[OpenCV] Motion detection succeeded at (%d, %d)", motionBall.x,
              motionBall.y);
        ...
        if (confidence > bestConfidence) {
          ...
          bestResult[@"radius"] =
              @(10.0); // Default radius for motion detection
          bestResult[@"isDetected"] = @(YES);
          bestResult[@"confidence"] = @(confidence);
      ...
      NSLog(@"[OpenCV] ERROR: Motion detection failed with exception: %@",
            exception.reason);
    }
  }
  ...
  if (bestConfidence > 0.0) {
    NSLog(@"[OpenCV] Unified detection succeeded using %@ method with "
          @"confidence %.3f",
...
static cv::Mat uiImageToMat(UIImage *image) {
  ...
  CGContextRef contextRef = CGBitmapContextCreate(
      mat.data, cols, rows, 8, mat.step[0], colorSpace,
...
static std::vector<Target> detectTargets(const cv::Mat &frame,
  ...
  std::vector<Target> circularTargets =
      detectCircularTargets(enhancedFrame, adaptiveRanges);
  ...
  if (tapeRects.empty()) {
    ...
    cv::findContours(pinkMask, pinkContours, cv::RETR_EXTERNAL,
                     cv::CHAIN_APPROX_SIMPLE);
    ...
    for (const auto &contour : pinkContours) {
      ...
      if (area > 300) { // Filter out small noise
        allPinkPoints.insert(allPinkPoints.end(), contour.begin(),
                             contour.end());
      }
    }
...
detectCircularTargets(const cv::Mat &frame,
  ...
  if (!circles.empty()) {
    ...
    for (const auto &circle : circles) {
      ...
      if (center.x >= 0 && center.x < frame.cols && center.y >= 0 &&
          center.y < frame.rows && radius > 0) {

        // More aggressive color matching with lower thresholds for outdoor
        bool isYellowTarget =
            isColorMatch(hsv, center, radius, adaptiveRanges.yellowLow,
                         adaptiveRanges.yellowHigh, 15);
        bool isRedTarget1 =
            isColorMatch(hsv, center, radius, adaptiveRanges.redLow1,
                         adaptiveRanges.redHigh1, 15);
        bool isRedTarget2 =
            isColorMatch(hsv, center, radius, adaptiveRanges.redLow2,
                         adaptiveRanges.redHigh2, 15);
        ...
        if (isYellowTarget) {
          ...
          target.boundingBox = cv::Rect(center.x - radius, center.y - radius,
                                        2 * radius, 2 * radius);
...
// Helper function to detect targets by contours (fallback for outdoor
// conditions)
static std::vector<Target>
detectTargetsByContours(const cv::Mat &frame,
                        const ColorThresholds::AdaptiveRanges &adaptiveRanges) {
  ...
  cv::Mat yellowMask;
  cv::inRange(hsv, adaptiveRanges.yellowLow, adaptiveRanges.yellowHigh,
              yellowMask);
  ...
  for (const auto &contour : yellowContours) {
    ...
    if (area > 2000 && area < 50000) { // Stricter area
      ...
      if (aspectRatio > 0.9 && aspectRatio < 1.1) {
        ...
        if (isColorMatch(hsv, center, radius, adaptiveRanges.yellowLow,
                         adaptiveRanges.yellowHigh, 30)) {
          Target target;
          target.isCircular = true;
          target.center = center;
          target.radius = radius;
  ...
  for (const auto &contour : redContours) {
    ...
    if (area > 2000 && area < 50000) { // Stricter area
      ...
      if (aspectRatio > 0.9 && aspectRatio < 1.1) {
        cv::Point center(rect.x + rect.width / 2, rect.y + rect.height / 2);
        int radius = std::max(rect.width, rect.height) / 2;
        if (isColorMatch(hsv, center, radius, adaptiveRanges.redLow1,
                         adaptiveRanges.redHigh1, 30) ||
            isColorMatch(hsv, center, radius, adaptiveRanges.redLow2,
                         adaptiveRanges.redHigh2, 30)) {
          Target target;
          target.isCircular = true;
          target.center = center;
          target.radius = radius;
...
static bool isColorMatch(const cv::Mat &hsv, const cv::Point &center,
  ...
  double percentage =
      (cv::countNonZero(mask) * 100.0) / (roiMat.rows * roiMat.cols);
...
static void drawDetectedTarget(cv::Mat &frame, const Target &target) {
  if (target.isCircular) {
    ...
    cv::putText(
        frame, "TARGET " + std::to_string(target.targetNumber),
...
static cv::Point detectBallByShape(const cv::Mat &frame) {
  ...
  cv::HoughCircles(
      ...
      gray.rows /
          6,   // min distance between centers (reduced for closer detection)
  ...
  if (!circles.empty()) {
    ...
    for (const auto &circle : circles) {
      ...
      cv::Rect roi =
          cv::Rect(std::max(0, x - radius), std::max(0, y - radius),
      ...
      if (roi.width > 0 && roi.height > 0) {
        cv::Mat roiMat = gray(roi);
        cv::Mat roiBinary;
        cv::threshold(roiMat, roiBinary, 0, 255,
          ...
          if (largestContour != contours.end()) {
            ...
            if (perimeter > 0) {
              ...
              if (circularity > MIN_CIRCULARITY) {
                NSLog(@"[OpenCV] Shape detection: Valid ball at (%d, %d) with "
                      @"radius %d, circularity %.3f",
      ...
      if (area > MIN_BALL_AREA && area < MAX_BALL_AREA) {
        ...
        if (aspectRatio > 0.9 && aspectRatio < 1.1) {
          ...
          if (!isTarget) {
            ...
            if (isColorMatch(hsv, center, std::max(rect.width, rect.height) / 2,
                             range.first, range.second, 30)) {
              if (center.x > 0 && center.x < frame.cols && center.y > 0 &&
                  center.y < frame.rows) {
                // Calculate circularity
                double perimeter = cv::arcLength(contour, true);
                double circularity =
                    4 * M_PI * area / (perimeter * perimeter + 1e-6);
...
static bool detectSoccerBall(const cv::Mat &frame, cv::Point &ballCenter,
                             float &ballRadius) {
  NSLog(@"[OpenCV] detectSoccerBall called with frame size: %dx%d", frame.cols,
        frame.rows);
  ...
  cv::inRange(blurred, cv::Scalar(0, 0, 180), cv::Scalar(180, 50, 255),
              maskWhite); // white
  cv::inRange(blurred, cv::Scalar(0, 0, 0), cv::Scalar(180, 255, 60),
              maskBlack); // black
  ...
  for (const auto &contour : contours) {
    ...
    if (area < 500 || area > 20000) {
      NSLog(@"[OpenCV] Contour rejected - area: %.1f (not in range 500-20000)",
            area);
      continue;
    }
    ...
    if (circularity < 0.7) {
      NSLog(@"[OpenCV] Contour rejected - circularity: %.3f (below 0.7)",
            circularity);
      continue;
    }
    ...
    NSLog(@"[OpenCV] Contour candidate - area: %.1f, circularity: %.3f, score: "
          @"%.1f, center: (%d, %d), radius: %.1f",
  ...
  if (bestScore > 0) {
    ...
    NSLog(@"[OpenCV] Soccer ball found - center: (%d, %d), radius: %.1f, "
          @"score: %.1f",
...
detectBallByMotion(const cv::Mat &gray,
  ...
  for (const auto &contour : contours) {
    ...
    if (area > 150 && area < 5000) { // Filter by size
      ...
      if (center.x > 0 && center.x < gray.cols && center.y > 0 &&
          center.y < gray.rows) {
        if (area > maxArea) {
          maxArea = area;
          bestCenter = center;
